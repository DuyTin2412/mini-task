...
export default function UsersPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Lỗi khi lưu user');
    } finally {
  ...
  async function handleDeleteConfirm() {
    ...
    setIsSubmitting(true);
    try {
      await apiFetch(`/users/${selectedUser.id}`, { method: 'DELETE' });
      closeModal();
      fetchUsers();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Lỗi khi xóa user');
    } finally {
      setIsSubmitting(false);
    }
  }
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 text-gray-800 font-sans">
      ...
      <div className="flex justify-between items-end mb-6 border-b border-gray-100 pb-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">User</h1>
        </div>
        <button
          onClick={openAdd}
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={4} className="py-16 text-center text-gray-500">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="font-medium text-gray-500">Chưa có user nào</p>
                      <p className="text-xs text-gray-400 mt-1">Bấm "Add User" để tạo tài khoản mới.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                users.map((user) => (
                  <tr key={user.id} className="hover:bg-gray-50 transition-colors group">
                    <td className="py-4 px-6 text-center text-gray-400 font-mono text-xs">#{user.id}</td>
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center justify-center gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => openEdit(user)} title="Chỉnh sửa" className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-md transition-all">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                          </svg>
                        </button>
                        <button onClick={() => openDelete(user)} title="Xóa" className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-md transition-all">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden pointer-events-auto border border-gray-100">
              <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <h2 className="text-lg font-semibold text-gray-900">
                  {selectedUser ? 'Cập nhật User' : 'Add User mới'}
                </h2>
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">
                    Password {selectedUser ? <span className="text-gray-400 text-xs font-normal">(để trống nếu không đổi)</span> : <span className="text-red-500">*</span>}
                  </label>
                  <input
                    type="password"
                    value={password}
                ...
                <div className="flex justify-end gap-3 pt-4 mt-2 border-t border-gray-50">
                  ...
                  <button type="submit" disabled={isSubmitting} className="px-5 py-2 text-sm font-medium text-white bg-gray-900 hover:bg-gray-800 rounded-lg shadow-sm transition-colors disabled:opacity-70 disabled:cursor-not-allowed">
                    {isSubmitting ? 'Đang lưu...' : (selectedUser ? 'Lưu thay đổi' : 'Tạo mới')}
                  </button>
                </div>
              </form>
            </div>
      ...
      <ConfirmDialog
        ...
        title="Xóa tài khoản?"
        message={`Bạn có chắc muốn xóa user "${selectedUser?.username}" không? Hành động này không thể hoàn tác.`}
